    ...
    while index < chars.len() {
        ...
        if state.is_inside_string.is_some() {
            ...
            state.current_scopes.push(CssScope {
                ...
                scope_type: if state.is_inside_at_rule { "at-rule".to_string() } else { "selector".to_string() },
            });
            current_code.clear();
            state.is_inside_property_value = false;
        ...
        } else if !state.is_inside_property_value && !state.is_inside_at_rule && current_character == ':' {
